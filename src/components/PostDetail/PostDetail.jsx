...
const PostDetail = () => {
    ...
    const handleLike = async () => {
      ...
      if(isLiked) {
        await axiosClient.post(`/posts/${id}/unlike`)
      }
      else {
        ...
        isLoading ? (<div>Loading.....</div>) : (<div>
            ...
            <div className="text-white text-2xl m-1 dark:text-zinc-700" >
              <div className="flex items-center">
                ...
                <p className="block text-sm">{dayjs(postData.createdAt).locale('vi').format("DD MMM")}</p>
                </div>
              </div>
            ...
            {isLiked ? (
              ...
              {(postData.likes.length < 2) ? (
                <span className="ml-1" >{postData.likes.length} Like</span>)
          ...
          {(postData.comments.length < 2) ? (
            <Link to={`/posts/${postData._id}`} className="pl-4" >{postData.comments.length} Comment</Link>)
            : (
            ...
            <div className="flex mx-auto items-center justify-center mt-10">
                ...
                <div className="mt-2 ml-2">
                    <TextareaAutosize name="content" placeholder="Add a comment...." className="dark:bg-zinc-200 bg-zinc-600 text-slate-300 dark:text-slate-800 placeholder-zinc-300" value={content} onChange={(e) => {setContent(e.target.value)}}/>
                </div>
